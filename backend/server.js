...
console.log(process.env.NYLAS_CLIENT_ID);
...
app.get("/nylas/read-emails", isAuthenticated, async (req, res) => {
  ...
  Nylas.with(user.accessToken)
    .messages.list({ limit: 10 })
    .then((messages) => {
      messages.forEach((message) => {
        labels2 = message.labels;
        labels2.forEach((label) => {
          if (label.displayName === "Feedback") {
            let htmlString = message.body;
            let plainText = htmlString.replace(/<[^>]+>/g, "");
            feedbackArr.push(plainText);
          }
        });
      });
      const json = JSON.stringify(feedbackArr);
      fs.writeFileSync("array.json", json);
